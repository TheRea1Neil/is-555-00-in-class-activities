...
deployable_model <- vetiver_model(extracted_workflow, 'rental_prices')

# Write the (deployable) vetiver model object to the board:
working_dir <- '~/Desktop/rental-prices/'
...
pin_board %>% pin_list()
pin_board %>% pin_versions('rental_prices')
...
deployed_model <- vetiver_pin_read(pin_board, 'rental_prices')
...
pin_board %>% 
  vetiver_pin_metrics(new_performance_metrics, "rental_metrics")

# Reusable plot from the pin_board "stash":
pin_board %>% pin_read("rental_metrics") %>% 
  vetiver_plot_metrics()
